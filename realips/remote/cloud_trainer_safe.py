...
import threading
import time
from realips.agent.sac import AutoSafeSAC, SACConfig
from realips.trainer.trainer_sac_safe import AutoSafeTrainer, AutoSafeTrainerParams
...
from realips.agent.AutoSafe import MATRIX_P, F
...
class CloudSystemParams(IpsSystemParams):
    def __init__(self):
        ...
        self.agent_params = SACConfig()
        self.trainer_params = AutoSafeTrainerParams()
...
class CloudSystem(IpsSystem):
    def __init__(self, params: CloudSystemParams):
        ...
        self.shape_observations += 4 # add tracking error as part of the observation

        self.agent = AutoSafeSAC(self.params.agent_params, self.shape_observations, 1, MATRIX_P, F)
        self.trainer = AutoSafeTrainer(AutoSafeTrainerParams(), self.agent)

        self.edge_status_subscriber = self.redis_connection.subscribe(channel=self.params.redis_params.ch_edge_ready_update)
        ...
        if self.params.cloud_params.artificial_bandwidth != -1.0:
            packet = pickle.dumps([self.agent.actor.get_weights(), 0])
        ...
        for step in range(step_count):
            ...
            if training:
                if traj_segment.sequence_number == (last_seg.sequence_number + 1):
                    ...
                    self.trainer.store_experience(last_seg.observations, traj_segment.last_action, r,
    ...
    def optimize(self):
        ...
        while True:
            ...
            training_info = self.trainer.optimize()
            self.model_stats.add_training_info(training_info)
            optimize_times += 1
            self.trainable -= 1

    def waiting_edge_ready(self):
        weights = self.agent.actor.get_weights()
        self.send_weights_and_noise_factor(weights, 0)
